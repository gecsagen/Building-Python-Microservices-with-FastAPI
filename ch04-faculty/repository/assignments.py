...
from models.data.facultydb import faculty_assignments_tbl, student_bin_tbl
from models.data.faculty import Assignment, StudentBin
...
class AssignmentSubmissionRepository:
    def create_bin(self, stud_id: int, bin_id: int, faculty_id: int):
        try:
            student_bin = StudentBin(
                bin_id=bin_id, faculty_id=faculty_id, stud_id=stud_id
            )
