...
def signup(signup: Signup):
    try:
        ...
        tourist = Tourist(
            id=userid, login=login, date_signed=datetime.now(), booked=0, tours=list()
        )
...
def login(login: User, bg_task: BackgroundTasks):
    try:
        ...
        bg_task.add_task(
            audit_log_transaction, touristId=str(login.id), message="login"
        )
...
def login(username: str, password: str, bg_task: BackgroundTasks):
    tourist_list = [
        ...
        if tourist["login"]["username"] == username
        and tourist["login"]["password"] == password
    ]
