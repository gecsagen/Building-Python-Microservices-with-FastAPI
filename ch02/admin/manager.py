...
from places.destination import (
    Tour,
    TourBasicInfo,
    TourInput,
    TourLocation,
...
def remove_tour_destination(id: UUID):
    try:
        ...
        return JSONResponse(
            content={"message": "tour deleted"}, status_code=status.HTTP_202_ACCEPTED
        )
    except:
        return JSONResponse(
            content={"message": "tour does not exist"},
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def list_valuable_visitors():
    try:
        sort_orders = sorted(
            approved_users.items(), key=lambda x: x[1].booked, reverse=True
        )
...
def approve_login(userid: UUID):
    try:
        ...
        return JSONResponse(
            content={"message": "user approved"}, status_code=status.HTTP_200_OK
        )
    except:
        return JSONResponse(
            content={"message": "invalid operation"},
